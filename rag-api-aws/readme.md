...
|---|---|---|---|---|---|---|---|
| 1 | Frontend | EC2 (Next.js + Docker) | `[completar]` | Interfaz de usuario principal | `3000` → ALB `/` | 🟩 Operativo | Backend, ALB |
| 2 | Backend | EC2 (Node.js/Express + Docker) | `[completar]` | API REST, auth dual, lógica de negocio | `4000` → ALB `/api/*` | 🟩 Operativo | RDS, Firebase, SES, ALB |
| 3 | Editor de código | EC2 (Next.js + Docker) | `[completar]` | IDE donde el developer resuelve ejercicios | `3001` → ALB `/editor/*` | 🟩 Operativo | Frontend, Backend, ALB |
| 4 | RAG API | EC2 (FastAPI + Docker) | `[completar]` | Generación de preguntas + evaluación con IA | `8000` → ALB `/rag/*` | 🟩 Operativo | RDS, OpenSearch, LLM externos, ALB |
| 5 | Base de datos | Amazon RDS (PostgreSQL) | `[completar]` | Persistencia de usuarios, sesiones, evaluaciones, perfiles técnicos | `5432` | 🟩 Operativo | Backend, RAG API |
| 6 | Búsqueda vectorial | Amazon OpenSearch Service | `[completar]` | Indexación y *retrieval* de embeddings para RAG | Puerto HTTPS | 🟩 Operativo | RAG API |
| 7 | Correo transaccional | Amazon SES | `[completar]` | Envío de correos de recuperación de contraseña | API SES | 🟩 Operativo | Backend |
| 8 | Balanceador de carga | Application Load Balancer | `[completar]` | Enrutamiento por *path* hacia los 4 servicios | `443 / 80` público | 🟩 Operativo | Las 4 EC2 |

> ⚠️ **Importante:** Completa región, endpoints reales y puertos exactos consultando la consola de AWS antes de entregar. Actualiza el campo **Estado** según la verificación más reciente.
...
![Dominio OpenSearch](techmock-informe/capturas/03-consola-aws-opensearch/)
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/05-contenedores-docker/rag-docker.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/06-funcionalidad-admin/admin-dashboard.png`

---

#### Panel de Usuarios

*Captura pendiente: listado de usuarios registrados en la plataforma con filtros de búsqueda, estado de cuenta y opciones de gestión por usuario.*

> 📌 Nombre de archivo esperado: `techmock-informe/capturas/06-funcionalidad-admin/admin-usuarios.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/06-funcionalidad-admin/admin-sesiones.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/06-funcionalidad-admin/admin-preguntas.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/06-funcionalidad-admin/admin-tecnologias.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/06-funcionalidad-admin/admin-evaluaciones.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/06-funcionalidad-admin/admin-reclutamientos.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/06-funcionalidad-admin/admin-notificaciones.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/06-funcionalidad-admin/admin-perfil.png`

---

#### Tabla de Clasificación de Developers

*Captura pendiente: ranking global de developers con podio visual para los tres primeros, posición y puntaje de cada usuario, y opciones de filtrado por tecnología y período.*

> 📌 Nombre de archivo esperado: `techmock-informe/capturas/06-funcionalidad-admin/admin-ranking.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/07-funcionalidad-developer/developer-dashboard.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/07-funcionalidad-developer/developer-perfil.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/07-funcionalidad-developer/developer-progreso.png`

---

#### Errores Cometidos en la Entrevista

*Captura pendiente: panel de errores recurrentes del developer, con categoría del error, frecuencia de aparición y recursos para su corrección.*

> 📌 Nombre de archivo esperado: `techmock-informe/capturas/07-funcionalidad-developer/developer-errores.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/07-funcionalidad-developer/developer-ranking.png`
...
> 📌 Nombre de archivo esperado: `techmock-informe/capturas/07-funcionalidad-developer/developer-ide.png`
...
|---|---|---|---|---|
| `[completar]` | Dockerización del frontend y backend con Dockerfiles multi-stage y overrides dev/prod | Alan — Grupo 6 | Errores de TypeScript: método `updateUserProfile` faltante en `AuthContext` y componente `Field` mal declarado dentro de un componente padre | `05-contenedores-docker/fronted-docker.png` |
| `[completar]` | Scaffolding de la capa de servicios del RAG API: rutas, orquestación RAG, modelos SQLAlchemy async y repositorios con *upsert* | Alan — Grupo 6 | Proveedor LLM incorrecto (Ollama en lugar de Groq) por entradas duplicadas en `.env` | `07-funcionalidad-developer/developer-evaluacion.png` |
| `[completar]` | Sistema de migraciones PostgreSQL idempotente (`schema_migrations` + script maestro) y despliegue en RDS | Alan — Grupo 6 | Garantizar idempotencia para permitir reejecuciones seguras en distintos entornos (local, staging, producción) | `02-consola-aws-rds/base.png` |
| `[completar]` | Refactor RAG API: corrección async/sync, `TabError` en `vector_store.py`, autenticación SigV4 con `boto3` y `requests-aws4auth` para OpenSearch | Alan — Grupo 6 | Fallos intermitentes por mezcla de código síncrono y asíncrono en la capa de *retrieval* | `03-consola-aws-opensearch/` |
| `[completar]` | Configuración del ALB con 4 *listener rules* y *target groups*; ajuste de `basePath: "/editor"` en Next.js | Alan — Grupo 6 | Los *health checks* del editor fallaban porque no reflejaban el prefijo `/editor` tras activar `basePath` | `01-consola-aws-ec2-alb/reglas.png` |
| `[completar]` | Implementación de autenticación dual Firebase + JWT y flujo de recuperación de contraseña vía SES con tokens de un solo uso | Alan — Grupo 6 | Configuración de credenciales IAM para SES desde EC2 usando `fromInstanceMetadata()` sin exponer claves en `.env` | `04-consola-aws-ses/identidades.png` |
| `[completar]` | Pruebas de integración end-to-end: login → selección de tecnología → generación de pregunta RAG → resolución en IDE → evaluación del código | Alan — Grupo 6 | Sincronización del flujo de datos entre el frontend, la RAG API y el backend para mantener el estado de la sesión activa | `07-funcionalidad-developer/developer-ide.png` |

> 📌 Para completar las fechas, ejecuta `git log --pretty=format:"%ad %s" --date=short` en cada repositorio. Agrega filas adicionales si existen más hitos relevantes.
...
- [ ] Completar pruebas end-to-end del flujo RAG: generación de preguntas → evaluación de respuestas → registro de errores → recomendaciones.
- [ ] Documentar todas las variables de entorno (`.env.example`) requeridas por cada servicio, sin exponer credenciales reales en el repositorio.
- [ ] Agregar la captura de Docker del servicio RAG API (`rag-docker.png`).
- [ ] Agregar capturas de funcionalidad del sistema (secciones 8.6 y 8.7) con las interfaces reales de Admin y Developer.
...
- [ ] Completar las regiones y endpoints reales en la Tabla de Infraestructura (sección 4).
...
| 3.0 | 17/06/2026 | Informe profesionalizado: descripción previa de cada imagen, sección completa de funcionalidad del sistema para roles Admin y Developer, flujo principal del sistema, tablas de listener rules y mejoras de formato general |
