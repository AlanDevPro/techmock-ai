...
/**
 * Fetch del resultado de análisis desde el backend usando sesionId.
 * El backend es la única fuente de verdad — nada viaja por query params.
 */
async function fetchResultado(sesionId: string): Promise<AnalisisPayload> {
  const res = await fetch(
    `${API_BASE}/api/v1/sesion/${sesionId}/analisis`,  // ← NUEVO ENDPOINT
    { cache: "no-store" }
  );

  if (!res.ok) {
    if (res.status === 404) {
      throw new Error("Sesión no encontrada. Puede que el análisis aún no esté disponible.");
    }
...
export default function AnalisisPage() {
  // ✅ useParams: obtiene sessionId desde la URL dinámica /analisis/[sessionId]
  // No usamos useSearchParams ni query params — todo viene del path
  ...
  useEffect(() => {
    // ✅ Guardia: sesionId debe existir y ser un UUID válido
    if (!sesionId) {
      setError("ID de sesión no válido o ausente.");
      setLoading(false);
      return;
    }
        ...
        <div style={{ maxWidth: "920px", margin: "0 auto" }}>
          ...
          <div className="fi" style={styles.tabBar}>
            ...
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              <span style={{
                fontSize: "11px", color: nivelColor,
