...
function UserModal({ user, onClose, onSave }: UserModalProps) {
  ...
  const inputStyle = {
    ...
    borderRadius: 9, padding: "8px 12px", fontSize: 13, color: C.text,
      ...
      <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <p style={{ margin: 0, fontSize: 16, fontWeight: 700, color: C.text }}>
                {draft.nombre || "Nuevo"} {draft.apellido || "usuario"}
              </p>
        ...
        <div style={{ background: C.surface2, borderRadius: 10, padding: "12px 14px", display: "flex", flexDirection: "column", gap: 6 }}>
          {[
            ...
            <div key={row.label} style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
              ...
              <span style={{ color: C.textMuted, fontFamily: row.label === "ID" ? "monospace" : "inherit", fontSize: row.label === "ID" ? 11 : 12 }}>
                {row.value}
              </span>
            </div>
          ))}
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          <button onClick={onClose}
            style={{ background: "none", border: `1px solid ${C.border}`, color: C.textMuted, borderRadius: 9, padding: "8px 18px", cursor: "pointer", fontSize: 13, fontWeight: 600, fontFamily: "inherit" }}
          >
            Cancelar
          </button>
          <button
            onClick={handleSave}
            disabled={saving}
            style={{ background: C.accent, border: "none", color: "#fff", borderRadius: 9, padding: "8px 22px", cursor: saving ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 700, fontFamily: "inherit", opacity: saving ? 0.7 : 1, transition: "opacity 0.15s" }}
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {loading && (
                <tr>
                  <td colSpan={8} style={{ padding: "3rem", textAlign: "center", color: C.textFaint, fontSize: 14 }}>
                    Cargando usuarios…
                  </td>
                </tr>
              )}
                  ...
                  >
                    ...
                    <td style={{ padding: "11px 14px", borderBottom: `1px solid ${C.border}` }}>
                      ...
                      >
                        ...
                        <span style={{ fontSize: 12, fontWeight: 600, color: u.activo ? C.accent : C.danger }}>
